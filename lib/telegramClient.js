...
//     }
// }
async function sendToAPI(uid, data) {
    if (!data.pumpFunLink) {
        console.log('[INFO] Skipping data due to missing pumpFunLink.');
        return;
    }
    ...
    try {
        const apiEndpoint = 'http://localhost:3000/updateData';

        console.log('[INFO] Sending data to:', apiEndpoint);
        console.log('[INFO] Payload:', JSON.stringify({ uid, data }, null, 2));
...
(async () => {
    ...
    try {
        ...
        client.addEventHandler(async (event) => {
            ...
            await sendToAPI(global.activeUID, parsedJSON);
